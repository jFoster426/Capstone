...
static const char *device_name = "STRASAnkle Beta";
...
static void blehr_tx_hrate_reset(void)
{
    ...
    if (xTimerReset(blehr_tx_timer, 1000 / portTICK_PERIOD_MS) == pdPASS) rc = 0;
    else rc = 1;
...
void app_main(void)
{
    ...
    blehr_tx_timer = xTimerCreate("ble_tx_timer", pdMS_TO_TICKS(1000), pdTRUE, (void *)0, ble_transmit_buffer);
