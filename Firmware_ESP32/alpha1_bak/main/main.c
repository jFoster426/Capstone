                         ...
                         float *r, float *p)
{
    ...
    *r = s_compAngleX - f_compAngleX;
    *p = s_compAngleY - f_compAngleY;
}
...
void testFileBTSender(void * pvParameters)
{
    ...
    while (1)
    {
        ...
        // Handle the edge case when diff == 0 and testingFinished == true, by letting this specific case pass.
        if (diff <= 0 && testingFinished == false) continue;
        ...
        // 
        if (testingFinished == true && testFileIter == testFileUpTo)
        {
            strcpy((char *)case1, "EOF");
            while (bt.write_available == false) vTaskDelay(1 / portTICK_RATE_MS);
...
void testFileCollector(void * pvParameters)
{
    ...
    {
        // If not actively sending test file.
        // testingFirst will go false once the first pass complete (malloc done).
        // Also check for malloc successful.
        if (testing == false || testingFirst == true || testFileData == NULL)
        {
        ...
        // Also get the button states in this function since this task uses
        // a lot of the CPU resources.
        i2c_register_read_byte(IPU_I2C_ADDR, 0x0D, &btn1State);
        i2c_register_read_byte(IPU_I2C_ADDR, 0x0E, &btn2State);

        if (btn1State == 1 || btn2State == 1) testingFinished = true;
        ...
        if (testFileUpTo > 5000 || testingFinished == true)
        {
        ...
        testFileData[testFileUpTo++] = (torque_data >> 8) & 0xff;;
        testFileData[testFileUpTo++] = torque_data & 0xff;
    }
}
...
void app_main(void)
{
    ...
    while (1)
    {
        ...
        // falls below 80%.
        if (batteryPercentage < 80 && chargerConnected == true)
        {
        ...
        if (bt.read_available > 0)
        {
            ...
            if (testingReceived == true)
            {
                testingFinished = false;
                testingFirst = testingReceived;
                testing = testingFirst;
            ...
            {
                testingFinished = true;
                printf("stop test received from GUI.\n");
            }
        }
        ...
        if (statFileStateChanged == true)
        {
            ...
            // Only clear the variable if the transfer was successful.
            if (send_statFile() == 0)
            {
                ...
                // Update the testing variable to trigger the testFileBTSender and
                // testFileCollector task, but only after the statFile was sent.
                testing = testingFirst;
